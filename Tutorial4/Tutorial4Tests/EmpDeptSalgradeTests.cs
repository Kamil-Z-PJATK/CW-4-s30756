...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnDept30EmpsOrderedBySalaryDesc()
    {
        ...
        List<Emp> result = emps.Where(emps => emps.DeptNo==30).OrderByDescending(emp =>emp.Sal ).ToList(); 
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.Select(emp => new {emp.EName,emp.Comm}).Where(emp => emp.Comm is not null); 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.Where(emp => emp.Sal>emps.Where(emp2=>emp.DeptNo==emp2.DeptNo).Average(emp2=>emp2.Sal)).Select(emp=>emp.EName); 
